...
H.<c>=GF(2^128, modulus="primitive")
h=H.modulus()
...
def phi_RMFE24(v): 
	...
	for i in range(number_blocks-1):
		w.append(v[32*i:32*i+32])
	w.append(v[32*(number_blocks-1):])
	res=[]
	...
	k=len(v)
    	...
    	r=field_iso_desc(m,4,g,h,F,H,P,R)
...
def psi_RMFE24(w,k):
    ...
    for j in range(len(w)):
    	...
    	hred=listsum(m[0:16],[0]+m[16:])
        
    	
        
    	#Apply FFT
    	w1=binaryFFT(hred,4,B,data)
